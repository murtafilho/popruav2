...
@section('title', 'Gerenciar Permissions')
...
@section('header')
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-lg font-semibold flex-1 text-center">Gerenciar Permissions</h1>
        <div class="w-10"></div>
    </div>
@endsection
...
@section('content')
    <div class="h-full overflow-y-auto bg-gray-50 dark:bg-gray-900 p-4 transition-colors duration-200">
        ...
        <div class="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <a href="{{ route('admin.permissions.create') }}" class="bg-blue-500 dark:bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-600 dark:hover:bg-blue-700 text-sm font-medium transition-colors duration-200">
                Nova Permission
            </a>
        </div>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-sm font-medium text-gray-900 dark:text-gray-200">Roles</th>
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($permissions as $permission)
                            <tr class="odd:bg-gray-50 dark:odd:bg-gray-700/30 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors duration-200">
                                <td class="px-4 py-3 font-medium text-gray-900 dark:text-gray-200">{{ $permission->name }}</td>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <form action="{{ route('admin.permissions.destroy', $permission) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir esta permission?')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">Nenhuma permission cadastrada.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
